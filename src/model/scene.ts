...
module BP3D.Model {
  ...
  export class Scene {
    ...
    public addItem(itemType: number, fileName: string, metadata, position: THREE.Vector3, rotation: number, scale: THREE.Vector3, fixed: boolean) {
      itemType = itemType || 1;
      var scope = this;
      console.log('Adding item:', fileName);
      ...
      var loaderCallback = function (geometry: THREE.Geometry, materials: THREE.Material[]) {
        ...
        console.log('Geometry bounds:', {
          min: geometry.boundingBox ? geometry.boundingBox.min : 'not computed',
          max: geometry.boundingBox ? geometry.boundingBox.max : 'not computed'
        });
