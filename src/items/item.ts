...
module BP3D.Items {
  ...
  export abstract class Item extends THREE.Mesh {
    ...
    constructor(protected model: Model.Model, public metadata: Metadata, geometry: THREE.Geometry, material: THREE.MeshFaceMaterial, position: THREE.Vector3, rotation: number, scale: THREE.Vector3) {
      ...
      // Apply default dimensions if available
      if (metadata.defaultDimensions) {
        console.log('Applying dimensions for:', metadata.itemName);
        console.log('Default dimensions (mm):', metadata.defaultDimensions);
        
        var width = metadata.defaultDimensions.width / 10;  // convert mm to cm
        var height = metadata.defaultDimensions.height / 10;
        var depth = metadata.defaultDimensions.depth / 10;
        
        console.log('Converted dimensions (cm):', { width, height, depth });
        
        // Calculate scale factors
        var currentWidth = this.halfSize.x * 2.0;
        var currentHeight = this.halfSize.y * 2.0;
        var currentDepth = this.halfSize.z * 2.0;
        
        console.log('Current model dimensions (cm):', {
          width: currentWidth,
        ...
        console.log('Calculated scale factors:', {
          x: scale.x,
          y: scale.y,
          z: scale.z
        });
      ...
      if (scale != null) {
        ...
        var finalWidth = this.halfSize.x * 2.0;
        var finalHeight = this.halfSize.y * 2.0;
        var finalDepth = this.halfSize.z * 2.0;
        console.log('Final dimensions (cm):', {
          width: finalWidth,
          height: finalHeight,
          depth: finalDepth
        });
    ...
    /** Resize to the given dimensions in millimeters */
    public resize(height: number, width: number, depth: number) {
      ...
      var newDepth = depth / 10;
      
      // Current dimensions are in cm
      var currentWidth = this.halfSize.x * 2.0;
      var currentHeight = this.halfSize.y * 2.0;
      var currentDepth = this.halfSize.z * 2.0;
      
      var x = newWidth / currentWidth;
      var y = newHeight / currentHeight;
      var z = newDepth / currentDepth;
    ...
    public getHeight = function () {
      return this.halfSize.y * 2.0 * 10;  // Convert from cm to mm
    }

    /** Returns width in millimeters */
    public getWidth = function () {
      return this.halfSize.x * 2.0 * 10;  // Convert from cm to mm
    }

    /** Returns depth in millimeters */
    public getDepth = function () {
      return this.halfSize.z * 2.0 * 10;  // Convert from cm to mm
    }
